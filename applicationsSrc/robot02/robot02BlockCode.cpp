...
void Robot02BlockCode::processLocalEvent(EventPtr pev) {
			...
			switch(message->id){
				...
				case GO_MAP_MSG_ID:
					...
					if(!recieved){
	        			...
	        			info.str("");
	        			info << "SEND END MAP INSTANT TO " << recvInterface->connectedInterface->hostBlock->blockId;
		        		scheduler->trace(info.str(),hostBlock->blockId);
				...
				case GO_WAIT_BUFFER:{
					...
					info.str("");
					info << "WAIT BUFFER BY " << recvInterface->connectedInterface->hostBlock->blockId;
					scheduler->trace(info.str(),hostBlock->blockId);
					myColor = GREEN;
					robotBlock->setColor(myColor);
					authSend = false;
	...
	if((nbOfEnd == 0 && recieved == true && !robotBlock->isMaster) && authSend){
...
void Robot02BlockCode::SendWaitBuffer(P2PNetworkInterface *send){
	P2PNetworkInterface *p2p;
	stringstream info;
	p2p = send;

	if(p2p->connectedInterface){
		GoWaitBuffer *message = new GoWaitBuffer();
		scheduler->schedule(new NetworkInterfaceEnqueueOutgoingEvent(scheduler->now() + COM_DELAY, message, p2p));
	}
}

GoWaitBuffer::GoWaitBuffer(){
